...
-- Number of Rows including NULL values

SELECT 'director_mapping' AS 'Table Name', COUNT(ROW_COUNT()) AS No_of_Rows
FROM director_mapping 
UNION SELECT 'genre', COUNT(*)
FROM genre 
UNION SELECT 'movie', COUNT(*)
FROM movie 
UNION SELECT 'names', COUNT(*)
FROM names 
UNION SELECT 'ratings', COUNT(*)
FROM ratings 
UNION SELECT 'role_mapping', COUNT(*)
FROM role_mapping;

-- Number if Rows excluding NULL values

SELECT 
    table_name AS 'Table Name', table_rows 'Total Rows'
FROM
...
SELECT 
    ...
    COUNT(*) - COUNT(worlwide_gross_income) AS Worldwide_Gross_Income,
    COUNT(*) - COUNT(languages) AS Language,
...
-- Second Part: total number of movies released each month
...
ON g.movie_id = m.id
WHERE year = 2019  -- Here only 2017, 2018 and 2019 data is there, hence we are considering 2019 as last year.
GROUP BY genre
...
(
SELECT 
    movie_id, COUNT(movie_id) AS movie_count
FROM
    genre
GROUP BY movie_id
...
RANK() OVER (ORDER BY COUNT(DISTINCT movie_id) DESC) genre_rank
FROM genre
GROUP BY genre
)
SELECT *
...
FROM
    ...
    ratings r ON r.movie_id = d.movie_id
        INNER JOIN
    genre g ON g.movie_id = d.movie_id, top_genre
